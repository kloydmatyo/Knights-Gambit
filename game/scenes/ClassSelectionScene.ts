...
export class ClassSelectionScene extends Phaser.Scene {
  ...
  private detailsPanel: Phaser.GameObjects.GameObject[] = [];
  ...
  private createDetailsPanel(characterClass: CharacterClass) {
    ...
    if (this.detailsPanel.length > 0) {
      this.detailsPanel.forEach(obj => obj.destroy());
      this.detailsPanel = [];
    }
    ...
    // Class header
    const emoji = this.getClassEmoji(characterClass.name);
    const header = this.add.text(panelX, panelY + 20, `${emoji} ${characterClass.name.toUpperCase()} ${emoji}`, {
      fontSize: "26px",
      color: "#ffe66d",
      fontFamily: "Courier New, monospace",
    }).setOrigin(0.5);
    this.detailsPanel.push(header);

    // Description
    const description = this.add.text(panelX, panelY + 60, characterClass.description, {
      fontSize: "14px",
      color: "#a8a8a8",
      fontFamily: "Courier New, monospace",
      align: "center",
    ...
    this.detailsPanel.push(description);

    // Stats section
    const statsTitle = this.add.text(panelX, panelY + 110, "📊 STATS", {
      fontSize: "18px",
      color: "#4ecdc4",
      fontFamily: "Courier New, monospace",
    }).setOrigin(0.5);
    this.detailsPanel.push(statsTitle);

    // Build stats text with proper line breaks
    let statsLines = [
    ...
    if (characterClass.baseMana) {
      statsLines.push(`Mana: ${characterClass.baseMana} MP`);
    }

    const statsText = this.add.text(panelX, panelY + 155, statsLines.join('\n'), {
      fontSize: "15px",
      color: "#ffffff",
      fontFamily: "Courier New, monospace",
      align: "center",
    ...
    this.detailsPanel.push(statsText);

    // Skills section
    const skillsTitle = this.add.text(panelX, panelY + 240, "⚡ CLASS SKILLS", {
      fontSize: "18px",
      color: "#f39c12",
      fontFamily: "Courier New, monospace",
    }).setOrigin(0.5);
    this.detailsPanel.push(skillsTitle);

    // Get skills from SkillManager
    const skills = this.getClassSkills(characterClass.name as CharacterClassName);
    let skillY = panelY + 275;

    skills.forEach((skill, index) => {
      // Skill name
      const skillName = this.add.text(panelX - 240, skillY, `${index + 1}. ${skill.name}`, {
        fontSize: "15px",
        color: "#ffe66d",
        fontFamily: "Courier New, monospace",
        fontStyle: "bold",
      });
      this.detailsPanel.push(skillName);

      // Skill type badge
      const typeColor = skill.type === "active" ? "#2ecc71" : "#9b59b6";
      const skillType = this.add.text(panelX - 240, skillY + 22, `[${skill.type.toUpperCase()}]`, {
        fontSize: "11px",
        color: typeColor,
        fontFamily: "Courier New, monospace",
        backgroundColor: skill.type === "active" ? "#1e5631" : "#4a235a",
        padding: { x: 5, y: 2 },
      });
      this.detailsPanel.push(skillType);

      // Cooldown info next to type
      if (skill.cooldown > 0) {
        const cooldown = this.add.text(panelX - 150, skillY + 22, `CD: ${skill.cooldown}`, {
          fontSize: "11px",
          color: "#4ecdc4",
          fontFamily: "Courier New, monospace",
          backgroundColor: "#16213e",
        ...
        this.detailsPanel.push(cooldown);
      }

      // Skill description
      const skillDesc = this.add.text(panelX - 240, skillY + 45, skill.description, {
        fontSize: "12px",
        color: "#c8c8c8",
        fontFamily: "Courier New, monospace",
        wordWrap: { width: 460 },
      ...
      this.detailsPanel.push(skillDesc);
